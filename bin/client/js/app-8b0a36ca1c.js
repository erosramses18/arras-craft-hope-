...
/***/function (module, exports) {
		...
		function drawPoly(context, centerX, centerY, radius, sides) {
			...
			if (!sides) {
				...
				for (var _i10 = 0; _i10 < sides; _i10++) {
					var _theta2 = _i10 / sides * 2 * Math.PI;
					var _x19 = centerX + radius * Math.cos(_theta2 + angle);
	...
	function drawHealth(x, y, instance, ratio, alpha) {
		...
		if (instance.nameplate && instance.id !== _gui.playerid) {
			...
			if (instance.name !== "\0") {
				...
				instance.render.textobjs[0].draw('arrascraft.io', x, y - realSize - 30, 16, color.lavender, 'center');
		...
		return function (ratio) {
...

			{
				...
				for (var _i13 = messages.length - 1; _i13 >= 0; _i13--) {
					...
					drawBar(_x29 - msg.len / 2, _x29 + msg.len / 2, _y2 + height / 2, height, color.black);
					// Draw the text
					ctx.globalAlpha = Math.min(1, msg.alpha);
					msg.textobj.draw(_text, _x29, _y2 + height / 2, height - 4, color.guiwhite, 'center', true);
					// Iterate and move
					_y2 += vspacing + height;
					if (msg.status > 1) {
...

			{
				...
				var save = _len7;
				var _x30 = -spacing - 2 * _len7 + statMenu.get() * (2 * spacing + 2 * _len7);
				...
				_gui.skills.forEach(function drawASkillBar(skill) {
					...
					if (cap) {
						...
						drawBar(_x30 + _height / 2, _x30 - _height / 2 + _len7 * ska(cap), _y3 + _height / 2, _height - 3 + config.graphical.barChunk, color.black);
						drawBar(_x30 + _height / 2, _x30 + _height / 2 + (_len7 - gap) * ska(cap), _y3 + _height / 2, _height - 3, color.grey);
						drawBar(_x30 + _height / 2, _x30 + _height / 2 + (_len7 - gap) * ska(level), _y3 + _height / 2, _height - 3.5, col);
						// Blocked-off area
						if (blocking) {
							ctx.lineWidth = 1;
							ctx.strokeStyle = color.grey;
							for (var j = cap + 1; j < _max; j++) {
								drawGuiLine(_x30 + (_len7 - gap) * ska(j), _y3 + 1.5, _x30 + (_len7 - gap) * ska(j), _y3 - 3 + _height);
							}
						}
						// Vertical dividers
						...
						for (var _j = 1; _j < level + 1; _j++) {
							drawGuiLine(_x30 + (_len7 - gap) * ska(_j), _y3 + 1.5, _x30 + (_len7 - gap) * ska(_j), _y3 - 3 + _height);
						}
						// Skill name
						_len7 = save * ska(_max);
						var textcolor = level == maxLevel ? col : !_gui.points || cap !== maxLevel && level == cap ? color.grey : color.guiwhite;
						text.skillNames[ticker - 1].draw(name, Math.round(_x30 + _len7 / 2) + 0.5, _y3 + _height / 2, _height - 5, textcolor, 'center', true);
						// Skill key
						text.skillKeys[ticker - 1].draw('[' + ticker % 10 + ']', Math.round(_x30 + _len7 - _height * 0.25) - 1.5, _y3 + _height / 2, _height - 5, textcolor, 'right', true);
						if (textcolor === color.guiwhite) {
							// If it's active
							global.clickables.stat.place(ticker - 1, _x30, _y3, _len7, _height);
						}
						// Skill value
						if (level) {
							text.skillValues[ticker - 1].draw(textcolor === col ? 'MAX' : '+' + level, Math.round(_x30 + _len7 + 4) + 0.5, _y3 + _height / 2, _height - 5, col, 'left', true);
						}
						// Move on 
						_y3 -= _height + _vspacing;
					}
				});
				global.clickables.hover.place(0, 0, _y3, 0.8 * _len7, 0.8 * (global.screenHeight - _y3));
				if (_gui.points !== 0) {
					// Draw skillpoints to spend
					text.skillPoints.draw('x' + _gui.points, Math.round(_x30 + _len7 - 2) + 0.5, Math.round(_y3 + _height - 4) + 0.5, 20, color.guiwhite, 'right');
				}
			}

			{
				...
				var _height2 = 25;
				var _x31 = (global.screenWidth - _len8) / 2;
				var _y4 = global.screenHeight - spacing - _height2;

				ctx.lineWidth = 1;
				// Handle exp
				// Draw the exp bar
				drawBar(_x31, _x31 + _len8, _y4 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x31, _x31 + _len8, _y4 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x31, _x31 + _len8 * _gui.__s.getProgress(), _y4 + _height2 / 2, _height2 - 3.5, color.gold);
				// Draw the class type
				text.class.draw('Level ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name, _x31 + _len8 / 2, _y4 + _height2 / 2, _height2 - 4, color.guiwhite, 'center', true);
				_height2 = 14;
				_y4 -= _height2 + _vspacing2;
				// Draw the %-of-leader bar
				drawBar(_x31 + _len8 * 0.1, _x31 + _len8 * 0.9, _y4 + _height2 / 2, _height2 - 3 + config.graphical.barChunk, color.black);
				drawBar(_x31 + _len8 * 0.1, _x31 + _len8 * 0.9, _y4 + _height2 / 2, _height2 - 3, color.grey);
				drawBar(_x31 + _len8 * 0.1, _x31 + _len8 * (0.1 + 0.8 * (max ? Math.min(1, _gui.__s.getScore() / max) : 1)), _y4 + _height2 / 2, _height2 - 3.5, color.green);
				// Draw the score
				text.score.draw('Score: ' + handleLargeNumber(_gui.__s.getScore()), _x31 + _len8 / 2, _y4 + _height2 / 2, _height2 - 2, color.guiwhite, 'center', true);
				// Draw the name
				ctx.lineWidth = 4;
				text.name.draw(player.name, Math.round(_x31 + _len8 / 2) + 0.5, Math.round(_y4 - 10 - _vspacing2) + 0.5, 32, color.guiwhite, 'center');
			}

			{
				...
				var _height3 = _len9;
				var _x32 = global.screenWidth - _len9 - spacing;
				...
				roomSetup.forEach(function (row) {
					...
					row.forEach(function (cell) {
						...
						drawGuiRect(_x32 + j++ * _len9 / _W, _y5 + _i14 * _height3 / _H, _len9 / _W, _height3 / _H);
					});
					_i14++;
				});
				ctx.fillStyle = color.grey;
				drawGuiRect(_x32, _y5, _len9, _height3);
				minimap.forEach(function (o) {
					if (o[2] === 17) {
						ctx.fillStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.globalAlpha = 0.8;
						drawGuiRect(_x32 + o[0] / global.gameWidth * _len9, _y5 + o[1] / global.gameHeight * _height3, 1, 1);
					} else {
						ctx.strokeStyle = mixColors(getColor(o[2]), color.black, 0.5);
						ctx.lineWidth = 1;
						ctx.globalAlpha = 1;
						drawGuiRect(_x32 + o[0] / global.gameWidth * _len9 - 1, _y5 + o[1] / global.gameWidth * _height3 - 1, 3, 3, true);
						ctx.lineWidth = 3;
					}
				});
				ctx.globalAlpha = 1;
				...
				_x32 + player.x / global.gameWidth * _len9 - 1, _y5 + player.y / global.gameWidth * _height3 - 1, 3, 3, true);
				ctx.lineWidth = 3;
				ctx.fillStyle = color.black;
				drawGuiRect(_x32, _y5, _len9, _height3, true); // Border

				drawGuiRect(_x32, _y5 - 40, _len9, 30);
				lagGraph(lag.get(), _x32, _y5 - 40, _len9, 30, color.teal);
				...
				text.debug[1].draw('Server Speed: ' + (100 * _gui.fps).toFixed(2) + '%' + (_gui.fps === 1 ? '' : ' OVERLOADED!'), _x32 + _len9, _y5 - 50 - 1 * 14, 10, _gui.fps === 1 ? color.guiwhite : color.orange, 'right');
				text.debug[0].draw(serverName, _x32 + _len9, _y5 - 50, 10, color.guiwhite, 'right');
			}

			{
				...
				var _height4 = 14;
				var _x33 = global.screenWidth - _len10 - spacing;
				var _y6 = spacing + _height4 + 7;
				text.lbtitle.draw('Leaderboard:', Math.round(_x33 + _len10 / 2) + 0.5, Math.round(_y6 - 6) + 0.5, _height4 + 4, color.guiwhite, 'center');
				var _i15 = 0;
				lb.data.forEach(function (entry) {
					drawBar(_x33, _x33 + _len10, _y6 + _height4 / 2, _height4 - 3 + config.graphical.barChunk, color.black);
					drawBar(_x33, _x33 + _len10, _y6 + _height4 / 2, _height4 - 3, color.grey);
					var shift = Math.min(1, entry.score / max);
					drawBar(_x33, _x33 + _len10 * shift, _y6 + _height4 / 2, _height4 - 3.5, entry.barcolor);
					// Leadboard name + score 
					text.leaderboard[_i15++].draw(entry.label + ': ' + handleLargeNumber(Math.round(entry.score)), _x33 + _len10 / 2, _y6 + _height4 / 2, _height4 - 5, color.guiwhite, 'center', true);
					// Mini-image
					var scale = _height4 / entry.position.axis,
					    xx = _x33 - 1.5 * _height4 - scale * entry.position.middle.x * 0.707,
			...
			//Draw upgrade menu
			{
				...
				if (_gui.upgrades.length > 0) {
					...
					var _height5 = _len11;
					var _x34 = glide * 2 * spacing - spacing;
					var _y7 = spacing;
					var xo = _x34;
					var xxx = 0;
					var yo = _y7;
					...
					_gui.upgrades.forEach(function drawAnUpgrade(model) {
						if (_y7 > yo) yo = _y7;
						xxx = _x34;
						global.clickables.upgrade.place(_i16++, _x34, _y7, _len11, _height5);
						// Draw box
						ctx.globalAlpha = 0.5;
						ctx.fillStyle = getColor(colorIndex);
						drawGuiRect(_x34, _y7, _len11, _height5);
						ctx.globalAlpha = 0.1;
						ctx.fillStyle = getColor(-10 + colorIndex++);
						drawGuiRect(_x34, _y7, _len11, _height5 * 0.6);
						ctx.fillStyle = color.black;
						drawGuiRect(_x34, _y7 + _height5 * 0.6, _len11, _height5 * 0.4);
						ctx.globalAlpha = 0.9;
						...
						var picture = getEntityImageFromMockup(model, _gui.color),
						    ...
						    xx = _x34 + 0.5 * _len11 - scale * position.middle.x * Math.cos(upgradeSpin),
						...
						// Tank name
						text.upgradeNames[_i16 - 1].draw(picture.name, _x34 + _len11 / 2, _y7 + _height5 - 6, _height5 / 8 - 3, color.guiwhite, 'center');
						ctx.strokeStyle = color.black;
						ctx.globalAlpha = 1;
						ctx.lineWidth = 3;
						drawGuiRect(_x34, _y7, _len11, _height5, true); // Border
						if (_ticker++ % 2) {
							_y7 -= _height5 + internalSpacing;
							_x34 += glide * (_len11 + internalSpacing);
						} else {
							_y7 += _height5 + internalSpacing;
						}
					});
	...
	var gameDrawDead = function () {
		...
		return function () {
			...
			text.time.draw('Time alive: ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.', x - 170, y + 55, 16, color.guiwhite);
